...
1. **PostToolUse hooks DO receive `tool_output`** (verified against the official hooks Input Schema: stdin includes `tool_name`, `tool_input.command`, `cwd`, AND `tool_output`). So a hook *could* parse the PR URL from `gh pr create`'s output. **This design deliberately does NOT** — `gh pr create` output format varies by flags, while `gh pr view --json number -q .number` is canonical. The hook fires precisely (via `if`) and Claude resolves the number canonically (constraint #3). Relying on Claude's resolution is more robust than parsing variable CLI output. (Correction: an earlier brainstorm note claimed PostToolUse can't see output — that was wrong; the chosen design is by preference, not necessity.)
